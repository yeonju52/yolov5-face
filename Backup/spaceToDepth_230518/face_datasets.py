...
class LoadFaceImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        #     H, W, C = img.shape
        #     block_size = 2
        #     x = img.reshape(H // block_size, W // block_size, 12)
        #     print(x.shape)
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        # top, bottom = int(round(dh - 0.1)), int(round(dh + 0.1)) -> ex) 114, 115
        ...
        if H % 2 == 1:
            img = cv2.copyMakeBorder(img, 1, 1, 0, 0, cv2.BORDER_CONSTANT, value=color)  # add border
        if W % 2 == 1:
            img = cv2.copyMakeBorder(img, 0, 0, 1, 1, cv2.BORDER_CONSTANT, value=color)  # add border
        # print(img.shape)
        H, W, C = img.shape
        block_size = 2
        img1 = img[: (H//block_size), : (W//block_size), :] # left up
        img2 = img[: (H//block_size), (W//block_size):, :] # right up
        img3 = img[(H//block_size) : , : (W//block_size), :] # left down
        img4 = img[(H//block_size) : , (W//block_size):, :] # right down
        img = img1
        img = np.concatenate([img, img2], axis=2)
        img = np.concatenate([img, img3], axis=2)
        img = np.concatenate([img, img4], axis=2)
        # print(img.shape)
    
        cv2.imshow('0~2', img[:, :, 6:9])
        cv2.waitKey(0)
        # cv2.imshow('0~2', img1)
        # cv2.waitKey(0)
        # cv2.imshow('3~5', img2)
...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
    ...
    left, right = int(round(dw - 0.1)), int(round(dw + 0.1))
    if img.shape[2] == 12:
        img1 = cv2.copyMakeBorder(img[:, :, 0:3], top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)
        img2 = cv2.copyMakeBorder(img[:, :, 3:6], top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)
        img3 = cv2.copyMakeBorder(img[:, :, 6:9], top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)
        img4 = cv2.copyMakeBorder(img[:, :, 9:12], top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)
        img = img1
        img = np.concatenate([img, img2], axis=2)
        img = np.concatenate([img, img3], axis=2)
        img = np.concatenate([img, img4], axis=2)
